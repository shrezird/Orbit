...
Remaining housekeeping to make this repository release-ready:

1. **Choose a license.** There is currently no `LICENSE` file, which means all rights reserved by default and others cannot legally use or fork the code. MIT or Apache-2.0 are common choices for a project like this; add the file and a `## License` section here.
2. **Add a `.gitignore`:**
   ...
   *.spec
   data/boards/
   data/plugins/plugins-config.json
   data/plugins/*/data.json
   data/plugins/*/__pycache__/
   ```